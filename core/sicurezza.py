...
FILE_LOG = os.path.join("logs", "sicurezza_log.json")
...
class SicurezzaJarvis:
    ...
    def registra(self, azione, risultato):
        evento = {"ora": datetime.datetime.now().strftime("%d/%m/%Y %H:%M:%S"), "azione": str(azione), "risultato": str(risultato)}
        self.log.append(evento)
        return self.salva_log()

    def autorizza(self, utente):
        return str(utente).strip().casefold() in {nome.casefold() for nome in self.utenti_autorizzati}

    def richiede_conferma(self, comando):
        testo = str(comando or "").casefold()
        comandi_protetti = ("elimina", "formatta", "spegni computer", "modifica sistema", "resetta", "ripristina configurazione")
        return any(parola in testo for parola in comandi_protetti)

    def stato(self):
        return {"protezione": self.protezione, "utenti": list(self.utenti_autorizzati), "eventi_registrati": len(self.log)}
