...
ApplicationWindow {
    ...
    Connections {
        ...
        function onStateChanged(payload) {
            try { root.setState(JSON.parse(payload)) } catch (e) {}
        }
    ...
    Rectangle {
        ...
        Repeater {
            model: [ ["CPU LOAD", root.cpuText], ["MEMORY", root.memoryText], ["STORAGE", root.diskText], ["NETWORK", root.networkText] ]
            Item {
                x: 14; y: 46 + index * 45; width: 220; height: 32
                Text { text: modelData[0]; color: "#4fa7b7"; font.pixelSize: 7; font.bold: true }
                Text { y: 12; text: modelData[1]; color: "#d8f7fb"; font.pixelSize: 12; font.bold: true }
                Rectangle { x: 120; y: 15; width: 90; height: 2; color: "#0b3340" }
                Rectangle { x: 120; y: 15; width: 90 * Math.min(1, Math.max(0.05, root.activity)); height: 2; color: "#187da1"; opacity: 0.8 }
            }
        }
        ...
        Repeater {
            model: [ ["KERNEL", root.kernelText], ["MEMORY", root.memoryState], ["DEVICES", root.deviceText], ["VOICE", root.voiceText] ]
            Item {
                x: 22; y: 242 + index * 20; width: 204; height: 14
                Text { text: modelData[0]; color: "#718f97"; font.pixelSize: 7 }
                Text { anchors.right: parent.right; text: modelData[1]; color: index === 3 && root.listening ? "#54f4ff" : "#b7dce3"; font.pixelSize: 7; font.bold: true }
            }
        }
        ...
        Repeater {
            ...
            Rectangle {
                width: 5
                height: 18 + Math.abs(Math.sin(root.t * 4.0 + index * 0.67)) * (58 * root.activity)
                x: 22 + index * 8
                y: 425 - height
                color: index % 4 === 0 ? "#54f4ff" : "#187da1"
                opacity: 0.35 + root.activity * 0.65
            }
        }
    ...
    Rectangle {
        ...
        Repeater {
            model: [ ["COMPUTER", "ONLINE"], ["NETWORK", "ONLINE"], ["BLUETOOTH", "READY"] ]
            Item {
                x: 22; y: 145 + index * 24; width: 204; height: 18
                Text { x: 10; text: modelData[0]; color: "#b7dce3"; font.pixelSize: 7; font.bold: true }
                Text { anchors.right: parent.right; text: modelData[1]; color: "#28ee70"; font.pixelSize: 7; font.bold: true }
                Rectangle { y: 17; width: parent.width; height: 1; color: "#092c37" }
            }
        }
        ...
        Repeater {
            ...
            Rectangle {
                width: 5
                height: 4 + Math.abs(Math.sin(root.t * (root.listening ? 10 : root.speaking ? 7 : 3) + index * 0.9)) * (root.listening || root.speaking ? 20 : 5)
        ...
        Text { x: 22; y: 420; width: 204; height: parent.height - 440; text: root.eventsText; color: "#8ebfc8"; font.pixelSize: 8; wrapMode: Text.Wrap; elide: Text.ElideLeft }
    }

    // Central neural reactor
    Item {
        id: core
        x: root.width / 2 - 215
        y: 92
        ...
        Rectangle { anchors.centerIn: parent; width: 382; height: 382; radius: 191; color: "transparent"; border.color: "#0a3441"; border.width: 1; opacity: 0.7 }
        ...
        Rectangle {
            anchors.centerIn: parent
            width: 86 + root.activity * 10
            height: width
            radius: width / 2
            color: root.listening ? "#65f9ff" : root.speaking ? "#49e6a2" : "#2bb9d3"
        ...
        // J.A.R.V.I.S. identity in the reactor center
        Text {
            anchors.centerIn: parent
        ...
        Text {
            ...
            y: 322
            text: root.listening ? "LISTENING" : root.speaking ? "SPEAKING" : "SYSTEM READY"
            color: root.listening ? "#54f4ff" : root.speaking ? "#28ee70" : "#b9eaf0"
            font.pixelSize: 11
            font.bold: true
        }
        Text {
            anchors.horizontalCenter: parent.horizontalCenter
            y: 342
            text: "NEURAL PROCESSING // " + Math.round(root.activity * 100) + "%"
            color: "#4fa7b7"
            font.pixelSize: 7
            font.bold: true
        }
    ...
    // Bottom command/response strip
    Rectangle {
        x: 286; y: root.height - 78; width: root.width - 572; height: 54
        color: "#04151e"
        border.color: "#0b4251"
        border.width: 1
        Text { x: 12; y: 7; text: "COMMAND"; color: "#4fa7b7"; font.pixelSize: 7; font.bold: true }
        Text { x: 12; y: 21; width: parent.width - 24; text: root.commandText; color: "#d8f7fb"; font.pixelSize: 10; elide: Text.ElideRight }
        Rectangle { x: parent.width - 84; y: 10; width: 60; height: 3; color: "#0b3340" }
        Rectangle { x: parent.width - 84; y: 10; width: 60 * root.activity; height: 3; color: "#54f4ff" }
    }

    // Decorative corner markers
    Item {
        anchors.fill: parent
        Rectangle { x: 16; y: 12; width: 34; height: 1; color: "#54f4ff" }
        Rectangle { x: 16; y: 12; width: 1; height: 34; color: "#54f4ff" }
        Rectangle { x: root.width - 50; y: 12; width: 34; height: 1; color: "#54f4ff" }
        Rectangle { x: root.width - 17; y: 12; width: 1; height: 34; color: "#54f4ff" }
        Rectangle { x: 16; y: root.height - 13; width: 34; height: 1; color: "#54f4ff" }
        Rectangle { x: 16; y: root.height - 46; width: 1; height: 34; color: "#54f4ff" }
        Rectangle { x: root.width - 50; y: root.height - 13; width: 34; height: 1; color: "#54f4ff" }
        Rectangle { x: root.width - 17; y: root.height - 46; width: 1; height: 34; color: "#54f4ff" }
    }
}