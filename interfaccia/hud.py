...
class HUDJarvis:
    ...
    def _run_tk(self):
        try:
            ...
            self.canvas = tk.Canvas(self.finestra, bg=self.BG, highlightthickness=0, bd=0)
    ...
    def _animazione(self):
        ...
        self._disegna()
        self.angolo = (self.angolo + 0.022) % (math.pi * 2)
        self.angolo2 = (self.angolo2 - 0.013) % (math.pi * 2)
        self.angolo3 = (self.angolo3 + 0.006) % (math.pi * 2)
        if self.ascolto or self.parlando:
            self.pulse = (self.pulse + 5) % 90
        elif self.pulse > 0:
            self.pulse *= 0.90
            if self.pulse < 1:
                self.pulse = 0
    ...
    def _disegna(self):
        c = self.canvas
        c.delete("all")
        w, h = max(c.winfo_width(), 1100), max(c.winfo_height(), 700)
        cx, cy = w * 0.50, h * 0.49
        r = min(w * 0.27, h * 0.39)
        self._griglia(c, w, h)
        self._topbar(c, w)
        ...
        c.create_text(157, 415, text="LISTENING..." if self.ascolto else "VOICE READY", fill=self.CYAN, font=("Helvetica", 13, "bold"))
        self._pannello(c, 20, 445, 300, 615, "CORE SYSTEMS")
        self._core_values(c, 40, 478)
        self._pannello(c, 20, 635, 300, h-105, "SHORTCUTS")
        self._shortcuts(c, 42, 675)
        ...
        self._pannello(c, w-320, 645, w-20, h-105, "VOICE INPUT")
        self._wave(c, w-300, 700, w-45, 735)
        c.create_text(w-173, 770, text="Hey Jarvis...", fill=self.CYAN, font=("Helvetica", 12))
        for extra, color, width in [(36, "#0A3444", 2), (24, self.BLUE, 2), (8, self.CYAN, 3)]:
            rr = r + extra
        ...
        c.create_arc(cx-r+2, cy-r+2, cx+r-2, cy+r-2, start=128, extent=66, style="arc", outline=self.YELLOW, width=7)
        for i in range(8):
            a = math.radians(128 + i*9)
            x, y = self._polar(cx, cy, r-3, a)
            c.create_oval(x-3, y-3, x+3, y+3, fill=self.YELLOW, outline="")
        core = r * 0.48
        c.create_oval(cx-core, cy-core, cx+core, cy+core, fill="#031019", outline="#39B8CD", width=3)
        c.create_oval(cx-core+15, cy-core+15, cx+core-15, cy+core-15, outline="#164C5D", width=2)
        c.create_oval(cx-core*.68, cy-core*.68, cx+core*.68, cy+core*.68, outline="#23778A", width=1)
        if self.pulse:
            pr = core*.65 + self.pulse
    ...
    def _topbar(self, c, w):
        ...
        c.create_text(w-22, 18, anchor="e", text="// UTENTE: SIMONE", fill=self.CYAN, font=("Helvetica", 9, "bold"))
    ...
    def _pannello(self, c, x1, y1, x2, y2, titolo):
        ...
        c.create_text(x1+15, y1+14, text=titolo, anchor="w", fill=self.WHITE, font=("Helvetica", 10, "bold"))
    ...
    def _system_values(self, c, x, y):
        vals = ["CPU", "RAM", "DISK", "NETWORK"]
        pct = [24, 52, 31, 74]
        for i, (name, value) in enumerate(zip(vals, pct)):
            yy = y + i*27
            ...
            c.create_rectangle(x+56, yy-5, x+56+1.33*value, yy+2, fill=self.BLUE, outline="")
            c.create_text(x+235, yy, text=f"{value}%", fill=self.CYAN, font=("Helvetica", 8))

    def _core_values(self, c, x, y):
        names = ["MEMORY", "SECURITY", "NETWORK", "AI ENGINE", "DEVICES", "HUD INTERFACE"]
        for i, name in enumerate(names):
            c.create_text(x, y+i*21, anchor="w", text=name, fill="#C4E8ED", font=("Helvetica", 8))
            c.create_text(275, y+i*21, anchor="e", text="ONLINE", fill=self.GREEN, font=("Helvetica", 8, "bold"))
    ...
    def _datetime_panel(self, c, x, y):
        c.create_text(x, y, anchor="w", text=datetime.datetime.now().strftime("%H:%M:%S"), fill=self.CYAN, font=("Helvetica", 24, "bold"))
        c.create_text(x, y+35, anchor="w", text=datetime.datetime.now().strftime("%A, %d %B %Y"), fill="#B9DDE2", font=("Helvetica", 8))
    ...
    def _wave(self, c, x1, y1, x2, y2):
        ...
        for i in range(width):
            amp = 5 + 12 * abs(math.sin((i+self.angolo*80)*0.19))
            yy = (y1+y2)/2 + math.sin(i*.33+self.angolo*9)*amp
            pts.extend([x1+i, yy])
        if len(pts) >= 4:
            c.create_line(*pts, fill=self.CYAN, width=2)

    def _console(self, c, x1, y1, x2, y2):
        c.create_rectangle(x1, y1, x2, y2, outline="#0A586B", width=1)
        lines = [" >> INIZIALIZZAZIONE JARVIS... COMPLETATA", " >> CARICAMENTO MODULI... COMPLETATO", " >> SISTEMI ONLINE", " >> PRONTO PER ISTRUZIONI"]
    ...
    def _ticks(self, c, cx, cy, r):
        for i in range(64):
            a = self.angolo*.15 + i*2*math.pi/64
            r2 = r+(17 if i%4==0 else 10)
            x1,y1=self._polar(cx,cy,r,a); x2,y2=self._polar(cx,cy,r2,a)
    ...
    def _rotor(self, c, cx, cy, r, rotation, count):
        for i in range(count):
            ...
            x1,y1=self._polar(cx,cy,r*.87,a); x2,y2=self._polar(cx,cy,r,a+.14)
            c.create_line(x1,y1,x2,y2,fill="#3DB8C9",width=2)

    def _polar(self, cx, cy, r, a):
        return cx+math.cos(a)*r, cy+math.sin(a)*r

    def _stato(self):
        if self.parlando: return "SPEAKING"
        if self.ascolto: return "LISTENING"
        return "SYSTEM ONLINE" if self.dati.get("stato", "Operativo") == "Operativo" else str(self.dati.get("stato", "STARTING")).upper()
