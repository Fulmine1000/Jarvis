...
class StatoJarvis:
    ...
    def cpu(self):
        if psutil:
            return {
                "utilizzo": psutil.cpu_percent(interval=0.1),
                "core": psutil.cpu_count() or 0,
            }
    ...
    def memoria_ram(self):
        if psutil:
            ...
            return {
                "totale_GB": round(ram.total / (1024 ** 3), 2),
                "usata_percentuale": ram.percent,
                "libera_GB": round(ram.available / (1024 ** 3), 2),
            }
    ...
    def disco(self):
        ...
        return {
            "totale_GB": round(disco.total / (1024 ** 3), 2),
            "usato_GB": round((disco.total - disco.free) / (1024 ** 3), 2),
            "libero_GB": round(disco.free / (1024 ** 3), 2),
        }
        ...
        if psutil:
            try:
                ...
                if batteria is not None:
                    return {
                        "percentuale": batteria.percent,
                        "collegato": batteria.power_plugged,
                    }
    ...
    def completo(self):
        return {
            ...
            "cpu": self.cpu(),
            "ram": self.memoria_ram(),
            "disco": self.disco(),
            "batteria": self.batteria(),
            "tempo_attivo": self.tempo_attivo(),
        }
