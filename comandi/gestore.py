...
class GestoreComandi:
    ...
    def _esegui_raw(self, c):
        ...
        if any(x in c for x in ("grazie", "grazie jarvis", "ottimo", "perfetto")):
        ...
        if c in ("stato sistema", "stato del sistema", "rapporto sistema", "diagnostica"):
            ...
            return (f"Diagnostica completata. Jarvis {s['stato']}, versione {s['versione']}. "
                    ...
                    f"Dispositivi registrati: {len(s['dispositivi'].get('dispositivi', []))}. "
                    f"Capacità operative attive.")
        ...
        if c.startswith("meteo") or c.startswith("tempo a "):
            ...
            localita = localita or "Napoli"
            return cap.meteo(localita) if cap else "Servizio meteo non disponibile."
